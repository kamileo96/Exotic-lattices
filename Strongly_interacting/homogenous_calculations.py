...
def phi_at_site_t_fn(vList,Tsort,ind, gs, t):
    phi =  0
    fn = 0
    for i in range(vList.shape[0]):
        v = vList[i]
...
def homogenous_cluster_calculate_phi_full(mu, J, adj_mat, niu, target_node, return_fn = True):
    ...
    for iteration in range(max_iterations):
        ...
        phi, fn = phi_at_site_t_fn(vList, Tsort, ind, ground_state, target_node)
    ...
    if return_fn: return phase_diagram_superfluid_mu_J, fn
...
def phaseDiagramDataParallel_H_tuple(G,target_node,niu_expected_edges,max_J=0.1,mu_min=0,mu_max=1,
                      point_number=100, point_number_J=None,parallel=True, max_global_flucs=5, max_loc_flucs=2,J_min=0,max_iterations=50, N_minus=-1, N_plus=+3, save_csv_path=None):
    ...
    if save_csv_path is not None:
        ...
        for J in tqdm(J_values, desc="Processing", leave=True):
            r_J = Parallel(n_jobs=-1)(delayed(homogenous_cluster_calculate_phi_H_tuple)(mu, J, H_tuple_mu1_J1, target_node, *basis_tup,max_iterations=max_iterations) for mu in mu_values)
    ...
    end_time = time.time()
    print(f"Calculation completed in {end_time - start_time:.2f} seconds")
    return (J_values, mu_values, phi_avg_values, fn_avg_values)
...
diag=phaseDiagramDataParallel_H_tuple(G,target_node,targetNiu,point_number=point_number,point_number_J=point_number_J ,max_iterations=max_iterations,
                                      J_min=J_min, max_J = max_J, max_loc_flucs=max_loc_flucs, N_minus=N_minus,N_plus=N_plus, save_csv_path=fp+name)
